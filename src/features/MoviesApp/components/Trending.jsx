...
function Trending() {
    ...
    useEffect(() => {
        setLoading(true)
        moviesApi
            .getListData(`trending/all/day?api_key=${TMDB_KEY}&region=VI&language=vi&page=${pageIndex}`)
            .then((resData) => {
                if (resData.status === 200 && resData.data.results) {
                    ...
                    showNotification('error', 'Xảy ra lỗi khi lấy dữ liệu trending.')
                }
            })
            .catch((error) => {
                showNotification('error', 'Xảy ra lỗi khi lấy dữ liệu trending.')
    ...
    return (
        <div className="bg-[#374955] min-h-[calc(100vh-64px)] relative">
            ...
            <div className="max-w-2xl mx-auto py-20 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8">
                <h2 className="text-xl font-bold tracking-tight text-gray-200">Xu hướng hôm nay</h2>
                {isLoading && (
